...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (rut && (!rut.value.trim() || !validarRut(rut.value.trim()))) {
  ...
  /* --- Contact form tabs --- */
  var contactoTabs = document.querySelectorAll('.contacto-tab');
  contactoTabs.forEach(function (tab) {
    tab.addEventListener('click', function () {
      var targetId = this.getAttribute('data-form');
      contactoTabs.forEach(function (t) { t.classList.remove('active'); });
      this.classList.add('active');
      document.querySelectorAll('.contacto-form-panel').forEach(function (panel) {
  ...
  /* --- Set min date for visit form --- */
  var visitaFecha = document.getElementById('visita-fecha');
  if (visitaFecha) {
    var today = new Date();
    var dd = String(today.getDate()).padStart(2, '0');
    var mm = String(today.getMonth() + 1).padStart(2, '0');
    var yyyy = today.getFullYear();
    visitaFecha.setAttribute('min', yyyy + '-' + mm + '-' + dd);
  }

  /* --- Visita form validation and submission --- */
  var visitaForm = document.getElementById('visitaForm');
  var submitVisitaBtn = document.getElementById('submitVisitaBtn');
  var visitaSuccess = document.getElementById('visitaSuccess');

  if (visitaForm) {
    visitaForm.addEventListener('submit', function (e) {
      e.preventDefault();

      visitaForm.querySelectorAll('.form-group').forEach(function (g) {
        g.classList.remove('error');
      });

      var isValid = true;

      var vNombre = document.getElementById('visita-nombre');
      if (!vNombre.value.trim()) {
        vNombre.closest('.form-group').classList.add('error');
        isValid = false;
      }

      var vEmail = document.getElementById('visita-email');
      if (!vEmail.value.trim() || !validarEmail(vEmail.value.trim())) {
        vEmail.closest('.form-group').classList.add('error');
        isValid = false;
      }

      var vTelefono = document.getElementById('visita-telefono');
      if (!vTelefono.value.trim()) {
        vTelefono.closest('.form-group').classList.add('error');
        isValid = false;
      }

      var vFecha = document.getElementById('visita-fecha');
      if (!vFecha.value) {
        vFecha.closest('.form-group').classList.add('error');
        isValid = false;
      }

      var vHorario = document.getElementById('visita-horario');
      if (!vHorario.value) {
        vHorario.closest('.form-group').classList.add('error');
        isValid = false;
      }

      if (!isValid) {
        var firstError = visitaForm.querySelector('.form-group.error');
        if (firstError) firstError.scrollIntoView({ behavior: 'smooth', block: 'center' });
        return;
      }

      submitVisitaBtn.textContent = 'Agendando...';
      submitVisitaBtn.disabled = true;
      submitVisitaBtn.style.opacity = '0.7';

      var formData = new FormData(visitaForm);

      fetch(visitaForm.action, {
        method: 'POST',
        body: formData,
        headers: { 'Accept': 'application/json' }
      })
        .then(function (response) {
          if (response.ok) {
            visitaForm.style.display = 'none';
            visitaSuccess.style.display = 'block';
          } else {
            alert('Hubo un error al agendar la visita. Por favor intenta nuevamente.');
            submitVisitaBtn.textContent = 'Agendar visita';
            submitVisitaBtn.disabled = false;
            submitVisitaBtn.style.opacity = '1';
          }
        })
        .catch(function () {
